...
class SiteSettings(models.Model):
    ...
    instagram_url = models.URLField(
        ...
        help_text="رابط صفحة إنستغرام كامل. اتركه فاضي = يختفي أيقونة إنستغرام.",
    )
    instagram_username = models.CharField(
        max_length=60, blank=True,
        help_text="اسم المستخدم بدون @ (اختياري — للعرض النصي فقط).",
    )
    location_ar = models.CharField(
        max_length=160, blank=True, default="عمّان، الأردن",
        help_text="الموقع كما يظهر بالفوتر وصفحات السياسات (مثال: عمّان، الأردن).",
    )
    location_en = models.CharField(max_length=160, blank=True, default="Amman, Jordan")
    support_email = models.EmailField(
        blank=True,
        help_text="بريد الدعم/التواصل (يظهر بصفحات السياسات إذا معبّأ).",
    )
    ...
    facebook_pixel_id = models.CharField(
        max_length=30, blank=True,
        help_text="رقم Meta/Facebook Pixel (بدون حروف). لو فاضي، ما رح ينحقن أي كود تتبع بالموقع.",
    )

    logo = models.ImageField(upload_to="branding/", blank=True)
    hero_image = models.ImageField(
        upload_to="branding/", blank=True,
        help_text="صورة الـ Hero بالصفحة الرئيسية. لو فاضية، يظهر رسم توضيحي بألوان الهوية.",
    )
    hero_image_focus = models.CharField(
        max_length=30, blank=True, default="center center",
        help_text="نقطة تركيز قص صورة الـ Hero (object-position)، مثال: center center أو 60% 40%.",
    )
    hero_video_url = models.URLField(
        blank=True,
        help_text="رابط فيديو mp4 للـ Hero (اختياري). لو معبّأ يُعرض بدل الصورة (صامت، تلقائي، "
    ...
    hero_stat_ar = models.CharField(max_length=120, blank=True, help_text="مثال: نوصّل يومياً داخل عمّان")
    ...
    working_hours_ar = models.CharField(
        max_length=120, blank=True, help_text="مثال: يومياً ٩ ص – ٩ م (يظهر بالفوتر إذا معبّأ)",
    )
    working_hours_en = models.CharField(max_length=120, blank=True)
    privacy_policy_url = models.URLField(blank=True, help_text="رابط سياسة الخصوصية (يظهر بالفوتر إذا معبّأ)")
    terms_url = models.URLField(blank=True, help_text="رابط الشروط والأحكام (يظهر بالفوتر إذا معبّأ)")
    ...
    primary_color = models.CharField(
        ...
        help_text="اللون البرتقالي الأساسي (أزرار، هيدر، تمييز)",
    )
    secondary_color = models.CharField(
        max_length=7, default="#00A850",
        help_text="اللون الأخضر (لوجو 'Low' بالخط اليدوي، تفاصيل ثانوية)",
    )
    soft_bg_color = models.CharField(
        max_length=7, default="#FFF3E6",
        help_text="لون الخلفية الكريمية الفاتحة (خلفيات السكاشن)",
    )

    # ---- استشارة أخصائي التغذية (upsell — تُقرأ من هون، مو Hardcoded) ----
    consultation_price_jod = models.DecimalField(
        max_digits=6, decimal_places=2, default=0,
        help_text="سعر جلسة استشارة التغذية (د.أ). يُنسخ للحجز وقت إنشائه.",
    )
    consultation_duration_min = models.PositiveIntegerField(
        default=20, help_text="مدة جلسة الاستشارة بالدقائق (تظهر بصفحة الاستشارات).",
    )
...
class Testimonial(models.Model):
    ...
    photo = models.ImageField("صورة العميل (اختياري)", upload_to="testimonials/", blank=True)
    location_ar = models.CharField("الموقع (اختياري)", max_length=80, blank=True)
    location_en = models.CharField("الموقع (إنجليزي)", max_length=80, blank=True)
    plan_label_ar = models.CharField("الخطة (اختياري)", max_length=80, blank=True)
    plan_label_en = models.CharField("الخطة (إنجليزي)", max_length=80, blank=True)
    is_featured = models.BooleanField("مميّز (يظهر بالصفحة الرئيسية)", default=True)
...
class HeroGoal(models.Model):
    ...
    """
    text_ar = models.CharField("النص (عربي)", max_length=60)
    text_en = models.CharField("النص (إنجليزي)", max_length=60, blank=True)
    order = models.PositiveIntegerField("الترتيب", default=0)
...
class HowItWorksStep(models.Model):
    ...
    title_ar = models.CharField("العنوان (عربي)", max_length=80)
    title_en = models.CharField("العنوان (إنجليزي)", max_length=80, blank=True)
    text_ar = models.CharField("الوصف (عربي)", max_length=200)
    text_en = models.CharField("الوصف (إنجليزي)", max_length=200, blank=True)
    image = models.ImageField("صورة الخطة", upload_to="how/", blank=True)
    image_focus = models.CharField(
        "نقطة تركيز القص", max_length=30, blank=True, default="center center",
        help_text="object-position، مثال: center center أو 50% 30%.",
    )
    order = models.PositiveIntegerField("الترتيب", default=0)
    is_active = models.BooleanField("مفعّل", default=True)
...
class FAQ(models.Model):
    question_ar = models.CharField("السؤال (عربي)", max_length=200)
    question_en = models.CharField("السؤال (إنجليزي)", max_length=200, blank=True)
    answer_ar = models.TextField("الجواب (عربي)")
    answer_en = models.TextField("الجواب (إنجليزي)", blank=True)
    category_ar = models.CharField(
        "التصنيف (عربي)", max_length=60, blank=True,
        help_text="اختياري — للتنظيم المستقبلي (خطط، وجبات، توصيل...). ما يظهر كفلتر إلا لو احتجناه.",
    )
    category_en = models.CharField("التصنيف (إنجليزي)", max_length=60, blank=True)
    order = models.PositiveIntegerField("الترتيب", default=0)
    show_on_homepage = models.BooleanField("يظهر بمقتطف الصفحة الرئيسية", default=True)
...
class SiteFeature(models.Model):
    ...
    icon = models.CharField("الأيقونة", max_length=20, choices=ICON_CHOICES, default="calories")
    title_ar = models.CharField("العنوان (عربي)", max_length=80)
    title_en = models.CharField("العنوان (إنجليزي)", max_length=80, blank=True)
    text_ar = models.CharField("الوصف (عربي)", max_length=180)
    text_en = models.CharField("الوصف (إنجليزي)", max_length=180, blank=True)
    order = models.PositiveIntegerField("الترتيب", default=0)
    is_active = models.BooleanField("مفعّل", default=True)
...
class Policy(models.Model):
    ...
    slug = models.SlugField(
        "المعرّف بالرابط", max_length=60, unique=True,
        help_text="يظهر بالرابط: /policies/<slug>/ (مثال: privacy, terms).",
    )
    title_ar = models.CharField("العنوان (عربي)", max_length=140)
    title_en = models.CharField("العنوان (إنجليزي)", max_length=140, blank=True)

    is_published = models.BooleanField(
        "منشورة", default=False,
        help_text="ما تنشرها إلا بعد ما يتأكد المحتوى القانوني. غير منشورة = "
                  "ما إلها رابط بالفوتر ولا صفحة (404).",
    )
    show_in_footer = models.BooleanField("تظهر بروابط الفوتر", default=True)
    order = models.PositiveIntegerField("الترتيب", default=0)

    version = models.CharField("رقم النسخة (اختياري)", max_length=20, blank=True)
    last_updated = models.DateField(
        "آخر تحديث", null=True, blank=True,
        help_text="التاريخ اللي يظهر بأعلى الصفحة ('آخر تحديث: ...').",
    )

    meta_description_ar = models.CharField("وصف SEO (عربي)", max_length=180, blank=True)
    meta_description_en = models.CharField("وصف SEO (إنجليزي)", max_length=180, blank=True)
...
class PolicySection(models.Model):
    ...
    heading_ar = models.CharField("عنوان القسم (عربي)", max_length=160, blank=True)
    heading_en = models.CharField("عنوان القسم (إنجليزي)", max_length=160, blank=True)

    body_ar = models.TextField("الفقرات (عربي)", blank=True, help_text="افصل كل فقرة بسطر فارغ.")
    body_en = models.TextField("الفقرات (إنجليزي)", blank=True)

    list_type = models.CharField("نوع القائمة", max_length=10, choices=LIST_CHOICES, default=LIST_NONE)
    list_items_ar = models.TextField("عناصر القائمة (عربي)", blank=True, help_text="عنصر واحد بكل سطر.")
    list_items_en = models.TextField("عناصر القائمة (إنجليزي)", blank=True)
